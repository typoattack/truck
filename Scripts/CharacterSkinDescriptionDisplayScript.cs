...
public class CharacterSkinDescriptionDisplayScript : MonoBehaviour {
        ...
        for (int g = 0; g <= 1; g++)
        {
            if (g == gender)
            {
                for (int s = 0; s < maxNumberOfSkins; s++)
                {
                    if (s == skin)
                    {
                        gameObject.transform.GetChild(g).GetChild(s).gameObject.SetActive(true);
                    }
                    else
                    {
                        gameObject.transform.GetChild(g).GetChild(s).gameObject.SetActive(false);
                    }
                }
            }
            else
